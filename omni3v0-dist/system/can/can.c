...
//返回值:0,初始化OK;
//    其他,初始化失败;
//remap PB8/PB9 as CAN_RX/CAN_TX
u8 CAN_Mode_Init(u8 tsjw,u8 tbs2,u8 tbs1,u16 brp,u8 mode)
{
	...
	//remap: CAN_RX mapped to PB8, CAN_TX mapped to PB9
  ...
  GPIOB->ODR|=1<<8; //PB8(CAN_RX) pull-up
	GPIOB->ODR|=1<<9; //PB9(CAN_TX) output high
					    
	RCC->APB1ENR|=1<<25;	//使能CAN时钟 CAN使用的是APB1的时钟(max:36M)
	CAN1->MCR=0x0000;		//退出睡眠模式(同时设置所有位为0)
	CAN1->MCR|=1<<0;		//请求CAN进入初始化模式
	while((CAN1->MSR&0x01)==0) //test INAK bit of CAN_MSR
	{
		i++;
	...
	while(CAN1->MSR&0x01) //test INAK bit of CAN_MSR 
	{
		i++;
	...
	CAN1->sFilterRegister[0].FR1=0x00000000;//32位ID of CAN_FiRx (i=0, x=1)
	CAN1->sFilterRegister[0].FR2=0x00000000;//32位MASK of CAN_FiRx (i=0,x=2), do not care.
...
{	   
	...
	CAN1->sTxMailBox[mbox].TIR|=id; //CAN_TIxR of datasheet	 
...
//send data to robomodule via CAN (标准帧,数据帧)		     
//msg:数据指针,最大为8个字节.
//返回值:0,成功;
//		 其他,失败;
u8 CAN_tx_RoboModule(u32 id, u8* msg)
{	
	u8 mbox;
	u16 i=0;	  	 						       
  mbox=CAN_Tx_Msg(id,0,0,8,msg);
	while((CAN_Tx_Staus(mbox)!=0x07)&&(i<0xFFF))
		i++;//等待发送结束
	if(i>=0XFFF)
		return 1;							//发送失败?
	return 0;										//发送成功;
...
//返回值:0,无数据被收到;
//		 其他,接收的数据长度;
u8 CAN_rx_RoboModule(u8 *buf, u32 *id)
{		   		   
	u8 ide,rtr,len; 
	if(CAN_Msg_Pend(0)==0)
		return 0;			//没有接收到数据,直接退出 	 
  CAN_Rx_Msg(0,id,&ide,&rtr,&len,buf); 	//读取数据
  //if(id!=0x7ff||ide!=0||rtr!=0)
	if(ide!=0||rtr!=0)
		len=0;		//接收错误	   
	return len;	
}
